        ...
        <div className="container px-8 mx-auto mt-16 lg:mt-32 min-h-[calc(100vh-20rem)]">
            {link ? (
                ...
                >
                    ...
                    <div className="flex flex-col items-center justify-center w-full gap-4 mt-4 sm:flex-row">
                        ...
                        <div className="w-full h-16 px-3 py-2 duration-150 border rounded sm:w-2/5 hover:border-zinc-100/80 border-zinc-600 focus-within:border-zinc-100/80 focus-within:ring-0 flex items-center justify-between">
                            ...
                            <Input
                                ...
                                name="reads"
                                id="reads"
                                className="w-full p-0 text-base bg-transparent border-0 appearance-none text-zinc-100 placeholder-zinc-500 focus:ring-0 sm:text-sm text-center"
                                value={reads}
                        ...
                        <div className="relative w-full h-16 px-3 py-2 duration-150 border rounded sm:w-2/5 hover:border-zinc-100/80 border-zinc-600 focus-within:border-zinc-100/80 focus-within:ring-0 ">
                            <div className="flex items-center justify-between h-full">
                                ...
                                <Input
                                    ...
                                    name="reads"
                                    id="reads"
                                    className="w-full p-0 text-base bg-transparent border-0 appearance-none text-zinc-100 placeholder-zinc-500 focus:ring-0 sm:text-sm ml-2 text-center"
                                    value={ttl}
                                    onChange={(e) => setTtl(e.target.valueAsNumber)}
                                />
                                <Select>
                                    <SelectTrigger className="border-0 text-zinc-100 hover:border-zinc-100/80 border-zinc-600 focus-within:border-zinc-100/80 focus-within:ring-0">
                                        <SelectValue placeholder={ttl === 1 ? "Day" : "Days"} />
                                    </SelectTrigger>
                                    <SelectContent className="bg-zinc-900 text-zinc-100">
                                        <SelectItem value={"60"} className="text-zinc-100">{ttl === 1 ? "Minute" : "Minutes"}</SelectItem>
                                        <SelectItem value={"3600"} className="text-zinc-100">{ttl === 1 ? "Hour" : "Hours"}</SelectItem>
                                        <SelectItem value={"86400"} className="text-zinc-100">{ttl === 1 ? "Day" : "Days"}</SelectItem>
                                    </SelectContent>
                                </Select>
                            </div>
                        </div>
                    ...
                    >
                        <span>{loading ? <Loader className="w-5 h-5 animate-spin" /> : "Share"}</span>
                    </Button>
                    <div className="mt-8">
